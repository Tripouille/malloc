...
show_alloc_tiny(char buffer[]) {
	...
	write(1, buffer, sprintf(buffer, "TINY : %p\n", (void*)g_memory_manager.tiny));
	for (t_zone_header * tiny = g_memory_manager.tiny; tiny != NULL; tiny = tiny->next_zone_header)
...
show_alloc_small(char buffer[]) {
	...
	write(1, buffer, sprintf(buffer, "SMALL : %p\n", (void*)g_memory_manager.small));
	for (t_zone_header * small = g_memory_manager.small; small != NULL; small = small->next_zone_header)
...
show_alloc_large(char buffer[]) {
	...
	write(1, buffer, sprintf(buffer, "LARGE : %p\n", (void*)g_memory_manager.large));
	for (t_zone_header * large = g_memory_manager.large; large != NULL; large = large->next_zone_header)
	{
