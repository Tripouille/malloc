...
static void *find_block_in_zone(size_t block_size, t_zone_header *zone_header)
{
...
static void * get_block_in_zone(size_t block_size, size_t zone_size, t_zone_header ** zone_header)
{
...
static void * get_large_zone(size_t block_size)
{
...
void *malloc(size_t size)
{
	...
	if (size <= TINY)
		return (get_block_in_zone(size, (sizeof(t_block_manager) + TINY) * BLOCK_PER_ZONE, &memory_manager.tiny));
	else if (size <= SMALL)
		return (get_block_in_zone(size, (sizeof(t_block_manager) + SMALL) * BLOCK_PER_ZONE, &memory_manager.small));
	else
